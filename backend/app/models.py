...
class SubscriptionPlan(Base):
    ...
    name = Column(String, nullable=False, unique=True)  # Launch, Accelerate, Scale, Essentials, Pro, Enterprise
    type = Column(String, nullable=False)  # subscription, self_hosted
    price_monthly = Column(Float, nullable=True)  # Monthly price in INR
    price_yearly = Column(Float, nullable=True)  # Yearly price in INR
    price_one_time = Column(Float, nullable=True)  # One-time price for self-hosted
    user_limit = Column(Integer, nullable=False)  # Base user limit
    additional_user_price = Column(Float, nullable=True)  # Price per additional user
    description = Column(Text, nullable=True)
    features = Column(Text, nullable=False)  # List of feature keys included (stored as JSON string)
...
class CompanySubscription(Base):
    ...
    status = Column(String, nullable=False, default="active")  # active, expired, cancelled, suspended
    start_date = Column(Date, nullable=False)
    end_date = Column(Date, nullable=True)  # null for self-hosted
    current_users = Column(Integer, default=0)
    max_users = Column(Integer, nullable=False)
    billing_cycle = Column(String, nullable=True)  # monthly, yearly, one_time
    next_billing_date = Column(Date, nullable=True)
    auto_renew = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
    
    # Fix the relationship by specifying foreign_keys
...
class PlanFeature(Base):
    ...
    description = Column(Text, nullable=True)
    category = Column(String, nullable=False)  # sales, marketing, support, general
    is_core = Column(Boolean, default=False)  # Core feature available in all plans
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class FeaturePreset(Base):
    ...
    features = Column(Text, nullable=False)  # {feature_key: true/false} (stored as JSON string)
...
class AuditLog(Base):
    ...
    before_data = Column(Text, nullable=True)  # JSON string (stored as text)
    after_data = Column(Text, nullable=True)  # JSON string (stored as text)
...
class CustomerLifecycleConfig(Base):
    ...
    statuses = Column(Text, nullable=False)       # e.g., ["Warm", "Hot", "Lost"] (stored as JSON string)
